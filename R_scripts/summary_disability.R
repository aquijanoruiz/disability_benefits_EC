...
var <- c("nonemployed", "sick", "prev_care", "hospital", "good_health", "better_health", "dis_manuela", "dis_transfer", 
         "bdh_transfer", "dis_degree_2", "dis_degree_3", "dis_degree_4", "dis_type_vision","dis_type_hearing", 
...
all_dis_id <- rbind(all_dis_id, c(sum(disability_ec$dis_id), sum(!disability_ec$dis_id), 
  sum(disability_ec$dis_id & disability_ec$sex_male), sum(!disability_ec$dis_id & disability_ec$sex_male), 
  sum(disability_ec$dis_id & disability_ec$sex_female), sum(!disability_ec$dis_id & disability_ec$sex_female)))

rownames(all_dis_id)[length(var) + 1] <- "observations"
